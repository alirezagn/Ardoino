...
#define ResetButton 10  // SD3 on EPS8266 nodemuc
...
void setup() {
  ...
  display.display();
  delay(1000);
  // loop for the wifi on_demand configuration
  // WiFiManager
  // Local intialization. Once its business is done, there is no need to keep it around
  //WiFiManager wifiManager;
  Serial.println("wifimanager line");
  ...
  //if (!wifiManager.startConfigPortal("WiFiClock")) {
  //  ESP.restart();
  //  delay(5000);
  ...
  //if (!wifiManager.startConfigPortal("OnDemandAP")) {
    ...
    display.println("Connect AP WiFiClock to setup");
    display.display();
  ...
  // DHT setup
  //*****************************
  dht.setup(DHTpin, DHTesp::DHT11);  //for DHT11 connect dht sencor to GPIO 17 = D5
  ...
  Wire.begin(4, 5);  // DISPLAY PINS D2 = GPIO 04 (SDA) && D1 = GPIO 05 (SCL)
  display.clearDisplay();
  display.setTextSize(1);
  display.setTextColor(WHITE);
  display.setCursor(12, 9);  // over (X) then down (Y)
  display.println("WiFi_Clock V1.0");
  display.display();
  delay(2000);
  display.clearDisplay();
...
void loop() {
  if (digitalRead(ResetButton) == LOW) {
    ...
    display.setTextColor(SSD1306_WHITE); // Draw white text
    display.setCursor(0, 0);     // Start at top-left corner
    display.println("Resetting Wifi Configuraion");
    display.display();
    delay(3000);
    wifiManager.resetSettings();
    ESP.restart();
...
void SetBrightness(time_t t)
{
  ...
  display.setCursor(0, 0);  // over (X) then down (Y)
  ...
  display.setTextSize(1);
  display.setTextColor(WHITE, BLACK);
  display.setCursor(0, 16);  // over (X) then down (Y)
  ...
  // Display Temperture and Humidity
  display.setTextSize(1);
  display.setTextColor(WHITE, BLACK);
